...
<br><br><br>
    ...
    $sql = "SELECT  DISTINCT email from admin  where DATE(timestamp) = DATE(NOW());";
    $sql1="Select distinct email from admin where Time(timestamp)>Time('10:15:59') and DATE(timestamp) = DATE(NOW());";
      ...
      <div class="sales-boxes">
                        ...
                        <!-- <a href="today_export_data.php" class="btn btn-success"style="margin-left: 88%;"> Download <i class="fa fa-download"></i></a> -->
                                    ...
                                    <tbody>
                                       <?php
                                        ...
                                        if($ser)
                                        {
                                            ...
                                            WHERE name LIKE '%$ser%' and DATE(timestamp) = DATE(NOW()) group by email; ";
                                            ...
                                            if(mysqli_num_rows($run)>0)
                                            {
                                                ...
                                                $lstatus='On location';
                                                $color1="green";
                                                if(($lt<28.5851280 || $lt>28.6097428) || ($ll<77.3120120 || $ll>77.3750800))
                                                {
                                                  ...
                                                  $color1='red';
                                                    ...
                                                    ?>
                                                        ...
                                                        <td><img src="/attendance/admin/<?= $row['image']; ?>" height="80px" width="150px" alt="Image"></td>
                                            ...
                                            $query="SELECT  DISTINCT * from admin  where DATE(timestamp) = DATE(NOW()) group by email; ";
                                        $run=mysqli_query($conn,$query);
                                        if(mysqli_num_rows($run)>0)
                                        {
                                            while($row=mysqli_fetch_assoc($run)){
                                                $time = new DateTime($row['timestamp']);
                                                $date = $time->format('j/n/Y');//echo "<br>";
                                                $time = $time->format('H:i:s');
                                                $l=$row['location'];
                                                $temp=explode(" ",$l);
                                                $lt=(double)$temp[0];
